...
constexpr size_t kSlotEntrySize = sizeof(uint16_t) /*offset*/ + sizeof(uint16_t) /*length*/;

// helper: compute data area size
constexpr std::size_t sizeOfDataArea() {
    // reserve: page_id (4), page_type (2), row_count (2), free_ptr (2), reserved (2) + slots
    constexpr std::size_t headerSize = sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint16_t);
...
struct Page {
    ...
    uint16_t free_space_ptr;// offset within data[] where next row will be appended
